...
- **关键任务自动多模型**：`multi_tasks = error / math_stem / detail / compare` 按 `groups.multi=true` 的分组且 `providers[].multi=true` 的模型并发（组间上限 `routing.max_multi_groups`，默认 3），结果交给 agnes 汇总成一份（agnes 不通则返回多份，由你兜底汇总）。
- **任务类型校验（task_check）**：识别返回后 agnes 判断 `--task` 是否合适，不合适输出 `TASK_CHANGE:xxx` 并用正确任务自动重跑一次（最多一次）。
- **额度熔断（按组双限值）**：日计数只对 `daily_group_limit > 0` 或 `daily_model_limit > 0` 的组生效（默认魔搭 组 2000 / 模型 500）；月 token 只对 `monthly_token_limit > 0` 的组生效（默认 internai 90M，按响应 usage 累计、跨月清零）；熔断生效值 = 限值 ×（1 − reserve_ratio）；超限自动沿链切下一家。
...
- `providers`：默认 7 家启用 + 可选 `internai` 第 8 家模板（`enabled: false`，填 key 并启用即自动进链）；每项含 `group`、`model`（可候选数组）、`api_key`、`enabled`、`multi`（是否参与多模型并发，默认 true）。
...
- `quota`：`enabled` / `reserve_ratio`（0.2）/ `min_interval_sec` / `cooldown_default_sec`（60）/ `cooldown_max_sec`（300）/ `state_file`；`groups`：组级行为表（`multi` / `daily_group_limit` 组日限 / `daily_model_limit` 模型日限 / `monthly_token_limit` 月 token 限 / `min_interval_sec`），0 = 不限。
- `tasks`：按任务覆盖 `{max_size: 长边像素, focus: 默认关注点提示词}`，缺失回退代码默认；`doctor`：`--doctor` 超时/并发（15/4）；`max_redirects`：下载重定向上限（5）。
